...
function mb(zb) {
    ...
    if (pb != null) {
        ...
        if (bc != 0) {
            ...
            timer[zb].max_res = ac; // resource max. capacity
            timer[zb].ms = 3600000 / bc; // delay generating resources
            dc = 100; // minimun delay generating resources
            if (timer[zb].ms < dc) {
                timer[zb].ms = dc;
            }
            timer[zb].node = pb;
...
function executeTimer(zb) {
    ...
    if (wb >= 0) {
        ...
        fb[zb].value = ec;
        timer[zb].node.innerHTML = sigfig(ec) + '/' + sigfig(timer[zb].max_res);
    }
}
